...
/**
 * RenderHudImage - Motor de Imagen HUD Reutilizable.
 * v6.4 - Fix: Visibilidad inmediata en Carousel y ADN unificado.
 */
const RenderHudImage = ({ projId, company, isCarousel = false }: { projId: string, company: string, isCarousel?: boolean }) => {
  return (
    <MotionDiv 
      key={projId}
    ...
    >
      <MotionDiv 
        ...
        whileHover={{ scale: 1.05, y: isCarousel ? 0 : "-5%" }}
        transition={{ duration: isCarousel ? 0.8 : 15, ease: "linear" }}
      >
        <Image
          src={COLLABORATION_IMAGES[projId]}
          alt={company}
          fill
          draggable={false}
          className="object-cover object-top grayscale sepia-[.7] contrast-125 brightness-75 transition-all duration-1000 select-none"
        />
      </MotionDiv>
      <div className="absolute inset-0 bg-amber-600/40 mix-blend-multiply shadow-[inset_0_0_100px_rgba(0,0,0,0.9)] pointer-events-none transition-opacity duration-1000 group-hover:opacity-80" />
    </MotionDiv>
  );
};
 ...
 */
export function CollaborationsArchive({ id }: { id?: string }) {
  ...
  const handleOpenLightbox = (projId: string) => {
    if (isDragging) return;
    setLightboxImage(COLLABORATION_IMAGES[projId]);
  ...
  const startMobileMarquee = useCallback(() => {
    ...
    const self = async () => {
      ...
      try {
        await mobileControls.start({ x: -halfWidth, transition: { duration, ease: "linear" } });
        if (viewMode === 'console') {
          mobileX.set(0);
  ...
  const startMarquee = useCallback(async () => {
    ...
    const self = async () => {
      ...
      const speed = 40; // Velocidad recalibrada v6.4
      const duration = remainingDistance / speed;
      try {
        await controls.start({ x: -halfWidth, transition: { duration, ease: "linear" } });
  ...
  useEffect(() => {
    if (viewMode === 'carousel' && !isTest) {
      ...
      x.set(0); mobileX.set(0);
      controls.stop(); mobileControls.stop();
    }
  }, [viewMode, isDragging, isMobileDragging, startMarquee, startMobileMarquee, x, mobileX, controls, mobileControls, isTest]);

  return (
    <HudPanel id={id} title={t("hud.collaborations_hub_intel_feed")} className="w-full overflow-hidden">
      <style jsx global>{`
        ...
        .amber-scrollbar {
          ...
          scrollbar-color: rgba(245, 158, 11, 0.3) transparent !important;
        }
      `}</style>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-4">
          ...
          <div className="flex bg-black/50 border border-white/10 rounded-sm p-1 backdrop-blur-md self-end md:self-auto shrink-0">
            <MotionButton
              ...
              className={cn(
                "relative flex items-center gap-2 px-4 py-2 font-mono text-[10px] font-black uppercase tracking-widest z-10 transition-colors duration-300 cursor-pointer",
            ...
            {viewMode === 'console' ? (
              ...
              >
                ...
                >
                  <div className="relative flex-grow w-full h-full rounded-3xl overflow-hidden border border-white/10 bg-black shadow-2xl">
                    <ExpandIndicator />
                    <RenderHudImage projId={activeProject.id} company={activeProject.company} />
                  </div>
                </div>
                ...
                <div className="lg:col-span-5 flex flex-col h-full gap-6 lg:gap-8 min-h-0">
                  <div className="hidden lg:flex flex-col gap-2 overflow-y-auto h-[35%] min-h-[200px] pr-2 flex-none amber-scrollbar">
                    {collaborations.map((project: Collaboration) => (
                      ...
                      >
                        <span className={cn("font-mono text-[8px] md:text-[9px] uppercase tracking-tighter mb-1", activeId === project.id ? "text-amber-500" : "text-white/40")}>{t("listItems.collaboration")}</span>
                        <span className={cn("font-bold text-xs md:text-base tracking-tight", activeId === project.id ? "text-white" : "text-white/60")}>{project.company}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="block lg:hidden w-full overflow-hidden relative py-2 flex-none border-b border-white/5 cursor-grab active:cursor-grabbing">
                    <MotionDiv
                      ...
                      onDragStart={() => { mobileControls.stop(); setIsMobileDragging(true); }}
                      onDragEnd={() => { setIsMobileDragging(false); startMobileMarquee(); }}
                    >
                      {[...collaborations, ...collaborations].map((project: Collaboration, idx: number) => (
                        <button
                          ...
                          className={cn(
                            ...
                            activeId === project.id ? "bg-amber-500/15 border-amber-500" : "bg-white/[0.03] border-transparent hover:bg-white/[0.08]"
                          )}
                        >
                          <span className={cn("font-mono text-[8px] uppercase tracking-tighter mb-1", activeId === project.id ? "text-amber-500" : "text-white/30")}>{t("listItems.collaboration")}</span>
                          <span className={cn("font-bold text-xs tracking-tight", activeId === project.id ? "text-white" : "text-white/60")}>{project.company}</span>
                        </button>
                      ))}
                    </MotionDiv>
                  </div>
                    ...
                    >
                      <div className="flex flex-col flex-none mb-6">
                        <div className="flex items-center gap-3 mb-4 flex-wrap">
                          <span className="font-mono text-[9px] md:text-[10px] text-amber-500 font-bold tracking-widest bg-amber-500/10 px-2 py-0.5 rounded-sm shrink-0">{t("hud.completed")}</span>
                          <div className="h-[1px] w-6 bg-white/20 shrink-0" />
                          <span className="font-mono text-[10px] md:text-[11px] font-black text-black bg-amber-500 px-3 py-1 rounded-sm uppercase tracking-widest shadow-[0_0_20px_rgba(245,158,11,0.4)] shrink-0">{activeProject.country}</span>
                        </div>
                      ...
                      <div className="flex-grow overflow-y-auto pr-2 pb-4 flex flex-col gap-6 relative amber-scrollbar">
                        ...
                        <div className="flex flex-wrap gap-2 flex-none">
                          {activeProject.techStack.map((tech: string) => (
                            <span key={tech} className="text-[8px] md:text-[9px] font-bold px-2.5 py-1 bg-white/5 text-white/50 border border-white/10 uppercase tracking-widest rounded-full">{tech}</span>
                          ))}
                        </div>
                      </div>

                      <div className="shrink-0 pt-3 border-t border-white/10">
                        <NextLink href={`/collaborations/${activeProject.id}`} className="inline-flex items-center justify-center gap-3 px-6 py-2.5 bg-gold-gradient rounded-sm shadow-[0_0_20px_rgba(245,158,11,0.2)] transition-all hover:scale-105 active:scale-95 group/btn w-full md:w-auto">
              ...
              >
                <MotionDiv 
                  ...
                  onDragStart={() => { controls.stop(); setIsDragging(true); }}
                ...
                >
                  {carouselItems.map((project: Collaboration, idx: number) => (
                    <MotionDiv 
                      ...
                      className="w-[85vw] md:w-[1100px] h-[820px] md:h-full flex-shrink-0 flex flex-col md:flex-row bg-[#050505] rounded-xl overflow-hidden border border-white/10 group shadow-2xl relative"
                    >
                      <div 
                        className="h-[280px] md:h-full w-full md:w-1/2 shrink-0 relative overflow-hidden border-b md:border-b-0 md:border-r border-white/10 cursor-pointer"
                        onClick={() => handleOpenLightbox(project.id)}
                      >
                        <ExpandIndicator />
                        <RenderHudImage projId={project.id} company={project.company} isCarousel />
                      </div>
                      ...
                      <div className="flex flex-col p-6 md:p-12 flex-grow w-full md:w-1/2 bg-black relative">
                        <div className="flex-none mb-4 md:mb-6 flex items-center gap-3 flex-wrap">
                          <span className="font-mono text-[9px] md:text-[10px] text-amber-500 font-bold tracking-widest bg-amber-500/10 px-2 py-0.5 rounded-sm shrink-0">{t("hud.completed")}</span>
                          <div className="h-[1px] w-6 bg-white/20 shrink-0" />
                          <span className="inline-block font-mono text-[10px] md:text-[11px] font-black text-black bg-amber-500 px-3 py-1 rounded-sm uppercase tracking-widest shadow-[0_0_20px_rgba(245,158,11,0.4)] shrink-0">{project.country}</span>
                        </div>
                        ...
                        <div className="flex-none h-[100px] md:h-[180px] flex flex-col justify-center border-b border-white/5 mb-2 md:mb-8 py-2 md:py-4">
                          <h3 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-black uppercase text-white leading-[0.9] tracking-tighter mb-1 md:mb-4">{project.company}</h3>
                          <div className="flex items-center gap-3">
                            <p className="font-mono text-amber-500 uppercase tracking-widest text-[9px] md:text-sm">{t(`projects.${project.id}.title`)}</p>
                            <span className="text-[8px] md:text-[10px] text-white/40 font-mono border-l border-white/10 pl-3">{project.duration}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 relative overflow-hidden">
                          <AnimatePresence mode="wait">
                            {expandedId === project.id ? (
                              <MotionDiv key="expanded-intel" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "100%" }} exit={{ opacity: 0, height: 0 }} className="h-full overflow-y-auto pr-2 amber-scrollbar">
                              ...
                              <MotionDiv key="static-intel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-4 md:gap-12">
                                <p className="text-xs md:text-lg text-white/40 leading-relaxed line-clamp-4 md:line-clamp-6 lg:line-clamp-none font-light">{t(`projects.${project.id}.clientOverview`)}</p>
                                <div className="flex flex-wrap gap-2 md:gap-4 pb-4">
                                  {project.techStack.slice(0, 5).map((t: string) => (
                                    <span key={t} className="text-[8px] sm:text-[9px] md:text-[11px] font-bold px-2.5 py-1 md:px-4 md:py-2 bg-white/5 border border-white/10 text-white/40 rounded-full uppercase tracking-widest">{t}</span>
                                  ))}
                                </div>
                              </MotionDiv>
                            )}
                        ...
                        <div className="flex-none min-h-[80px] md:min-h-[120px] flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/5 mt-auto">
                          <button onClick={(e) => { e.stopPropagation(); setExpandedId(expandedId === project.id ? null : project.id); }} className="text-[10px] md:text-xs font-mono font-bold text-amber-500/60 hover:text-amber-500 transition-colors uppercase tracking-[0.2em] flex items-center gap-1 shrink-0">
                            {expandedId === project.id ? t("hud.close_intel") : t("hud.open_intel")}
                          </button>
                          <NextLink href={`/collaborations/${project.id}`} className="inline-flex items-center gap-2 px-6 py-4 md:px-7 md:py-3.5 bg-gold-gradient rounded-sm text-black font-bold uppercase text-[10px] md:text-[11px] tracking-[0.1em] sm:tracking-[0.3em] shadow-[0_0_20px_rgba(245,158,11,0.2)] hover:scale-105 transition-all w-full sm:w-auto justify-center">
